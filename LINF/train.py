...
def train(train_loader, model, optimizer, epoch, patch=False):
    ...
    for batch in tqdm(train_loader, leave=False, desc='train'):
        ...
        wandb.log({'loss/total_loss': loss.item(), 'loss/log_p': nll, 'loss/pixel_loss': pixel_l, 'loss/vgg_loss': vgg_l}, step=(epoch-1)*iter_per_epoch + iteration)
...
#             writer.add_scalars('psnr', {'val': val_res}, epoch)
            wandb.log({'psnr/val': val_res}, step=epoch)
