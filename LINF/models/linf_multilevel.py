...
class LINF(nn.Module):
    ...
    class WaveletTransformLayer(nn.Module):
        def __init__(self, wavelet_name='haar', level=3):
            super().__init__()
        ...
        def forward(self, x):
            ...
            coeffs = pywt.wavedec2(x_np, self.wavelet, mode='zero', level=self.level)
            # Process coefficients to be compatible with torch tensor and concatenate
            coeffs_tensors = []
            for coeff in coeffs:
                if isinstance(coeff, tuple):
                    for subcoeff in coeff:
                        coeffs_tensors.append(torch.from_numpy(subcoeff).to(x.device))
                else:
                    coeffs_tensors.append(torch.from_numpy(coeff).to(x.device))
            return torch.cat(coeffs_tensors, dim=1)
    ...
    def __init__(self, encoder_spec, imnet_spec=None, flow_layers=10, num_layer=3, hidden_dim=256):
        ...
        # You might need to adjust the input dimension depending on the total number of coefficients
        layers = []
